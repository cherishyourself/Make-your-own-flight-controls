#include "fuzzypid.h"
#include "stdio.h"
fuzzy_pid fpproll,fpppitch,fppyaw;//对于各种角度的指针
fuzzy_pid * fproll;//翻滚角
fuzzy_pid * fppitch;//俯仰角
fuzzy_pid * fpyaw;//偏航角
float P_FPID_K,P_FPID_I,P_FPID_D,P_FPID_MAXI,P_FPID_MINI,P_FPID_MAXO,R_FPID_K,R_FPID_I,R_FPID_D,R_FPID_MAXI,R_FPID_MINI,R_FPID_MAXO;//FPID
...
static void Fuzzy_Calc(fuzzy_pid *vPID,float pv,float *deltaK)
{
  ...
  int indexE[2]={0,0};          //偏差隶属度索引 根据索引找到对应规则库中的位置
  float msE[2]={0,0};           //偏差隶属度
  int indexEC[2]={0,0};         //偏差增量隶属度索引
  float msEC[2]={0,0};          //偏差增量隶属度
  float qValueK[3];
 
  LinearQuantization(vPID,pv,qValue);//pv当前值  qValue偏差及其增量的量化值************************************************************模糊量化处理
 
  CalcMembership(msE,qValue[0],indexE);//计算msE偏差隶属度  用indexE隶属度索引记录位置****************************************用模糊控制规则进行模糊决策
  CalcMembership(msEC,qValue[1],indexEC);//计算msEC增量偏差隶属度  用indexEC隶属度索引记录位置
 
  qValueK[0]=msE[0]*(msEC[0]*ruleKp[indexE[0]][indexEC[0]]+msEC[1]*ruleKp[indexE[0]][indexEC[1]])//KP的解模糊化值**********************解模糊化
            +msE[1]*(msEC[0]*ruleKp[indexE[1]][indexEC[0]]+msEC[1]*ruleKp[indexE[1]][indexEC[1]]);
  qValueK[1]=msE[0]*(msEC[0]*ruleKi[indexE[0]][indexEC[0]]+msEC[1]*ruleKi[indexE[0]][indexEC[1]])//Ki的解模糊化值
            +msE[1]*(msEC[0]*ruleKi[indexE[1]][indexEC[0]]+msEC[1]*ruleKi[indexE[1]][indexEC[1]]);
  qValueK[2]=msE[0]*(msEC[0]*ruleKd[indexE[0]][indexEC[0]]+msEC[1]*ruleKd[indexE[0]][indexEC[1]])//Kd的解模糊化值
            +msE[1]*(msEC[0]*ruleKd[indexE[1]][indexEC[0]]+msEC[1]*ruleKd[indexE[1]][indexEC[1]]);
 //***********限幅操作，现在上面解模糊化之后的kp ki kd
...
float Fuzzy_pid(fuzzy_pid *vPID,float set,float get)  //pid，位式
{
    ...
    vPID->fuzzy_err[FUZZY_NOW]=set-get;            //得到当前偏差值
	  ...
	  Fuzzy_Calc(vPID,get,delta); //解模糊化
    Final_kp = vPID->fuzzy_kp+kpscal*delta[0];//计算最终的kp ki kd 
